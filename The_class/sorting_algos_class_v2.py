...
class SortingAlgo:
    ...
    Therefore, I used the above quick_sort function.'''
    @staticmethod
    def quick_sort_2(the_arr):
        if len(the_arr) < 2:
            return the_arr
        else:
            pivot = the_arr[0]
            less = [i for i in the_arr[1:] if i <= pivot]
            greater = [i for i in the_arr[1:] if i > pivot]
    ...
    def count_sort(self):
        ...
        for i in range(1, count_len): count[i] += count[i - 1]
...
if __name__ == '__main__':
    while True:
        ...
        thequickArray = SortingAlgo.quick_sort_2(UserArr)
        print(thequickArray)
        exit()
        ...
        TheArrayQuick = SortArr.quick_sort(UserArr)
